...
contract RFID_IC is ERC20{
    ...
    event UserRegistered(bytes8 hexcode, uint256 tokenbalance);
    ...
    mapping(bytes8 => UserInfo) public userData;
    //We use bytes8 to store Hexcode Strings

    function setUserData(bytes8 hexCode) public {
    ...
    function BindWallet(bytes8 hexCode) public {
    ...
    function getUserData(bytes8 hexCode) public view returns (address, uint256) {
        UserInfo memory info = userData[hexCode];
        return (info.walletAddress, info.tokenBalance);
    }

    function getUserWallet(bytes8 hexCode) public view returns (address) {
        UserInfo memory info = userData[hexCode];
        return info.walletAddress;
    }

    function getUserTokenBalance(bytes8 hexCode) public view returns (uint256) {
        UserInfo memory info = userData[hexCode];
        if (!isRegistered(hexCode)) {
    ...
    function RFID_Scanned(bytes8 hexCode) public {
        if (userData[hexCode].exists) {
    ...
    function isRegistered(bytes8 hexCode) public view returns(bool) {
        return userData[hexCode].withWallet;
    }


    // Function to allow users to spend and earn rewards
    function spendAndEarn(bytes8 hexCode, uint256 _amountSpent) external {
